...
sub delete {
    ...
    $delay->steps(
        ...
        sub {
            ...
            Clovershell::Exception::OpenAPI->throw({ error => $attached_clovers_count . ' clovers are attached to this tag', status => 409 }) if $attached_clovers_count;
        ...
        sub {
            ...
            Clovershell::Exception::OpenAPI->throw({ error => $err, status => 500 }) if $err;
            Clovershell::Exception::OpenAPI->throw({ error => 'Not found', status => 404 }) unless $r->rows;
        }
    );
}
